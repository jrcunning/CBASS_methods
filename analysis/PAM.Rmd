...
quantile(alldf$std.error_2020, 0.85, na.rm = T)
quantile(alldf$std.error_2021, 0.85, na.rm = T)
highconf <- alldf %>%
  filter(std.error_2021 < 0.2, std.error_2020 < 0.2)      # 0.3 filters to 54/67 = 80% of genets    #0.25 filters to 38/67 = 56%
# Plot raw ED50 values from 2021 vs. 2020
ggplot(highconf, aes(x = ed50_2020, y = ed50_2021)) +
  geom_point(aes(color = nursery, shape = nursery)) +
  geom_errorbar(aes(xmin = ed50_2020 - std.error_2020, xmax = ed50_2020 + std.error_2020), lwd = 0.1) +
  geom_errorbar(aes(ymin = ed50_2021 - std.error_2021, ymax = ed50_2021 + std.error_2021), lwd = 0.1) +
  geom_text(aes(label = geno), size = 5) +
...
rand <- function(n) {
  df <- drop_na(alldf) %>% slice_sample(n = n*2) %>%
    mutate(group = factor(c(rep(1, 5), rep(2, 5))))
  mod20 <- aov(ed50_2020 ~ group, data = df)
  tuk20 <- TukeyHSD(mod20)
  mod21 <- aov(ed50_2021 ~ group, data = df)
  tuk21 <- TukeyHSD(mod21)
  c(tuk20$group[c(1,4)], tuk21$group[c(1,4)])
}
res <- replicate(10000, rand(5))

res <- as_tibble(t(res)) %>%
  rename(diff.20 = V1, p.20 = V2, diff.21 = V3, p.21 = V4)
res %>%
  filter(p.20 < 0.05) %>%
  ggplot(aes(x = diff.20, y = diff.21)) +
  geom_point()
res %>%
  filter(p.20 < 0.05) %>%
  ggplot(aes(x = abs(diff.20), y = abs(diff.21))) +
  geom_point()

# Bland-Altman plot
alldf <- alldf %>%
  mutate(avg = (ed50_2021 + ed50_2020)/2,
         diff = ed50_2021 - ed50_2020)
ggplot(alldf, aes(x = avg, y = diff)) +
  geom_point()

mean_diff <- mean(alldf$diff, na.rm = T)
lower <- mean_diff - 1.96*sd(alldf$diff, na.rm = T)
upper <- mean_diff + 1.96*sd(alldf$diff, na.rm = T)
upper

# If select top X from 2020 data, what is chance they have significantly higher than average ED50 in 2021?
X <- 10
topX <- alldf %>%
    arrange(-ed50_2020) %>%
    slice(1:X)
...
hi <- alldf %>%
  mutate(group = geno %in% topX$geno)
mod <- lm(ed50_2021 ~ group, data = hi)
anova(mod)

emm <- emmeans::emmeans(mod, specs = "group")
emmeans::contrast(emm, "eff")
```
...
library(rptR)
long
out <- rpt(ed50 ~ nursery * year  + (1|geno), data = long, grname = "geno")
out
out

lowvar <- alldf %>% filter(std.error_2021 < 0.5, std.error_2020 < 0.25)
out <- rpt(ed50 ~ nursery * year + (1|geno), data = filter(long, geno %in% lowvar$geno), 
                                                            grname = c("geno", "Fixed"))
out
ggplot(alldf, aes(x = ed50_2020, y = ed50_2021, color = std.error_2021 < 0.5 & std.error_2020 < 0.25)) +
  geom_point()


#RC
# Mean within-subject sample variance
alldf <- alldf %>% rowwise() %>% mutate(var = var(c(ed50_2021, ed50_2020)))
var_w <- mean(alldf$var, na.rm = TRUE)
# Within-subject sample standard deviation
s_w <- sqrt(var_w)
# Coefficient of repeatability
rc <- 1.96 * sqrt(2) * s_w
rc   # 95% of the time, repeat measurements will differ by less than rc
```

```{r}
# Going back to .pim files and putting 2 AOIs per coral
# 9/6/21: 
#  GOT IT >>> pim file 1 is wrong corals / image saved incorrectly -- cannot get new data
#   GOT IT pim file 2 only contains top 3 out of 5 corals from the rack in the image
#   GOT IT pim file 3 is wrong corals
#   GOT IT pim file 36 is the SAME as 35 -- must have been saved incorrectly...
# 9/8/21
#   pim file 21 is the SAME as 20 -- / previous corals...AHA just figured this out.... go back to 9/6 to recover^^^
#   pim file 35: take FIRST of three rows (manually)
```
