...
# test <- alldf %>% drop_na() %>% mutate(mahal = joeyr::tidy_mahalanobis(std.error_2021^3, std.error_2020^3)) %>%
#   arrange(-mahal)
# ggplot(test, aes(x = sum.std.error, y = mahal)) +geom_point()
# ggplot(test, aes(x = std.error_2020, y = std.error_2021)) + geom_point(aes(color = sum.std.error >= 0.63))

highconf <- alldf %>%
  #filter(std.error_2021 < 0.5, std.error_2020 < 0.3)      # 0.3 filters to 54/67 = 80% of genets    #0.25 filters to 38/67 = 56%
  filter(sum.std.error < 0.63)
...
```{r}
# Get all pairwise differences between high confidence genos in 2020 and 2021
genos <- highconf$geno
diffs2020 <- outer(highconf$ed50_2020, highconf$ed50_2020, `-`)
dimnames(diffs2020) <- list(geno1 = genos, geno2 = genos)
diffs2021 <- outer(highconf$ed50_2021, highconf$ed50_2021, `-`)
