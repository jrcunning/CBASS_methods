...
runres.unadj <- tibble(groupsize = c(1,3,10,20)) %>%
  mutate(runout = future_map(groupsize, ~reprun(indf.unadj, 10000, groupsize = .x)),
         runout = map(runout, ~map_dfr(.x, bind_rows)))
# Fit model for re-test (mean) difference between groups as a function of initial test difference
runres.unadj <- runres.unadj %>%
  #WITH PREDICTION INTERVALS
  mutate(mod = map(runout, ~lm(meandiff.2 ~ absdiff.1, data = .x)),
         pred= map(mod, ~ data.frame(predict(.x, newdata = data.frame(absdiff.1 = seq(0, 2, 0.01)),
                                 interval = "predict", level = 0.9))),
         pred= map(pred, ~.x %>% mutate(data.frame(absdiff.1 = seq(0, 2, 0.01)))))
  #WITH BINOMIAL MODEL
  # mutate(mod = map(runout, ~glm(meandiff.2 > 0 ~ absdiff.1, data = .x, family = "binomial")),
  #        pred = map(mod, ~ data.frame(fit = predict(.x, newdata = data.frame(absdiff.1 = seq(0, 2, 0.01)),
  #                                                   type = "response"))),
  #        pred = map(pred, ~ .x %>% mutate(data.frame(absdiff.1 = seq(0, 2, 0.01)))))
# best way to calculate % of new observations that will be > 0? something weird about where the pred int crosses zero, because confidence should be 50% at 0, not 0....
runres.unadj



hi <- runres.adj %>%
  select(groupsize, mod) %>%
  crossing(conf = seq(0.1, 0.99, 0.01)) %>%
  mutate(int = map2(mod, conf, ~ data.frame(predict(.x, newdata = data.frame(absdiff.1 = seq(0, 2, 0.01)),
                                                    interval = "predict", level = .y)) %>%
                                            mutate(absdiff.1 = seq(0, 2, 0.01))),
         thresh = map_dbl(int, ~ .x %>% filter(lwr > 0) %>%
                           summarize(thresh = min(absdiff.1)) %>%
                           pull(thresh))) #%>%
  #select(groupsize, conf, thresh)

hi$int[[1]]

ggplot(hi, aes(x = thresh, y = conf)) +
...
runres.adj <- tibble(groupsize = c(1,3,10,20)) %>%
...
#   geom_ribbon(aes(y = fit, ymin = lwr, ymax = upr), data = allpred.unadj, alpha = 0.2, fill = "blue") +
#   geom_hline(aes(yintercept = 0), lty = 1, lwd = 0.25) +
#   coord_cartesian(ylim = c(-1.5, 1.5), xlim = c(0, 1.5)) +
#   theme_custom()

ggplot(allpred.unadj, aes(y = absdiff.1, x = fit)) +
  geom_line(aes(lty = factor(groupsize)))
  
  


thresh.unadj <- allpred.unadj %>%
  group_by(groupsize) %>%
  filter(lwr > 0) %>%
  summarize(thresh = min(absdiff.1))

thresh.adj <- allpred.adj %>%
  group_by(groupsize) %>%
  filter(lwr > 0) %>%
  summarize(thresh = min(absdiff.1))

ggplot(data = thresh.unadj, aes(x = groupsize, y = thresh)) +
  geom_point(color = "gray") +
  geom_line(color = "gray") +
  geom_point(data = thresh.adj, color = "black") +
  geom_line(data = thresh.adj, color = "black") +
...
### this is pretty cool ^^^^.... 
# maybe also compare to unadjusted ed50s
### YES ^^^ do these simulations with adjusted and unadjusted ED50s!!!

# # Need to just manually get groups with the biggest diffs...
# there are SO many random combinations that the biggest diff groups are not appearing in randomized simulations
# get top and bottom before running sims?
outwide <- out %>%
  pivot_wider(names_from = "date", values_from = c(ed50, ed50_adj)) 
top25 <- outwide %>%
  arrange(ed50_adj_202010) %>%
  tail(15) %>% drop_na(ed50_adj_202109)
bottom25 <- outwide %>%
  arrange(ed50_adj_202010) %>%
  head(15) %>% drop_na(ed50_adj_202109)

topbot <- bind_rows(top25, bottom25) %>%
  select(geno, `202010` = ed50_adj_202010, `202006` = ed50_adj_202006, `202109` = ed50_adj_202109) %>%
  pivot_longer(c(`202006`, `202109`)) %>% drop_na()
res <- reprun(topbot, 1000, groupsize = 1)
res <- map_dfr(res, bind_rows)
mod <- lm(meandiff.2 ~ absdiff.1, data = res)
pred <- data.frame(predict(mod, newdata = data.frame(absdiff.1 = seq(0, 2, 0.01)), 
                           interval = "predict", level = 0.9)) %>% mutate(absdiff.1 = seq(0, 2, 0.01))
thresh <- tibble(conf = seq(0.5, 1, 0.05)) %>%
  mutate(int = map(conf, ~ data.frame(predict(mod, newdata = data.frame(absdiff.1 = seq(0, 2, 0.01)),
                              interval = "predict", level = .x)) %>% mutate(absdiff.1 = seq(0, 2, 0.01)))) %>%
  mutate(thresh = map_dfr(int, ~ .x %>% filter(lwr > 0) %>% summarize(thresh = min(absdiff.1))))
ggplot(res, aes(x = absdiff.1, y = meandiff.2)) + geom_point(alpha = 0.1) +
  geom_smooth(method = "lm") +
  geom_ribbon(data = pred, aes(y = fit, ymin = lwr, ymax = upr), alpha = 0.2)


modp <- glm(meandiff.2 > 0 ~ absdiff.1, data = res, family = "binomial")
predp <- data.frame(fit = predict(modp, newdata = data.frame(absdiff.1 = seq(0, 2, 0.01)), 
                                  type = "response")) %>% mutate(absdiff.1 = seq(0, 2, 0.01))
ggplot(predp, aes(x = absdiff.1)) + 
  geom_line(data = predp, aes(y = fit), alpha = 0.2)
