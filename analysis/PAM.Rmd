...
# Function to pivot IPAM data to long form with column for AOI -- 2 AOIs per coral
ipam_convert2 <- function(data) {
  data %>% select_if(~ !any(is.na(.))) %>%
  pivot_longer(cols = starts_with("f") | starts_with("y")) %>%
  separate(name, into = c("var", "aoi"), sep = "(?<=[A-Za-z_])(?=[0-9])")
}
...
pam1 <- pam1 %>%
  ...
  select(file_id, date, time, aoi, position, var, value)
