...
reprun(indf.unadj, 100, groupsize = 20)
library(furrr)
plan(multiprocess)
# Run simulations on unadjusted ED50 data
runres.unadj <- tibble(groupsize = c(1,3,10,20)) %>%
  mutate(runout = future_map(groupsize, ~reprun(indf.unadj, 5000, groupsize = .x)),
...
runres.adj <- tibble(groupsize = c(1,3,10,20)) %>%
  mutate(runout = future_map(groupsize, ~reprun(indf.adj, 5000, groupsize = .x)),
         runout = map(runout, ~map_dfr(.x, bind_rows)))
# Fit model for re-test (mean) difference between groups as a function of initial test difference
...
#   geom_ribbon(aes(y = fit, ymin = lwr, ymax = upr), data = allpred.adj, alpha = 0.2, fill = "blue") +
#   geom_hline(aes(yintercept = 0), lty = 1, lwd = 0.25) +
...
top10 <- outwide %>%
  arrange(ed50_202010) %>%
  tail(10) %>% drop_na(ed50_202109)
bottom10 <- outwide %>%
  arrange(ed50_202010) %>%
  head(10) %>% drop_na(ed50_202109)
mean(bottom10$ed50_adj_202010)
mean(top10$ed50_adj_202010)
36.12825 - 35.2719
mean(bottom10$ed50_adj_202109)
mean(top10$ed50_adj_202109)
35.9325 - 35.30401
...
###############
# but what about looking at a *minimum* difference between groups instead of *mean difference*?
ranmin <- function(df, groupsize) {
  g <- indf %>% group_by(geno) %>% slice_sample(n=1) %>% ungroup() %>%
    slice_sample(n = 2 * groupsize)
  g <- g %>% mutate(group = c(rep(1, groupsize), rep(2, groupsize)))
  diffs <- outer(g$`202010`[1:groupsize], g$`202010`[(groupsize+1):(2*groupsize)], "-")
  # if group 1 are all higher, then all diffs are >0
  if (all(diffs > 0.25)) {
    mindiff.1 <- min(diffs)
    r <- g %>% group_by(group) %>%
      summarize(mean.2 = mean(value)) %>%
      summarize(across(2, diff))
    return(c(mindiff.1 = mindiff.1, meandiff.2 = -1 * r$mean.2))
  }
  if (all(diffs < -0.25)) {
    mindiff.1 <- min(abs(diffs))
    r <- g %>% group_by(group) %>%
      summarize(mean.2 = mean(value)) %>%
      summarize(across(2, diff))
    return(c(mindiff.1 = mindiff.1, meandiff.2 = r$mean.2))
  }
}
ranmin(indf, 3)

res <- replicate(10000, ranmin(indf, 3), simplify = FALSE)
res2 <- map_dfr(res, bind_rows)
# res2 <- res2 %>% 
#   mutate(absdiff.1 = abs(mindiff.1), meandiff.2 = meandiff.2 * sign(mindiff.1))
mod <- lm(meandiff.2 ~ mindiff.1, data = res2)
newdat <- data.frame(mindiff.1 = seq(0, 1.5, 0.1))
pred <- data.frame(predict(mod, newdata = newdat, interval = "predict", level = 0.9)) %>% bind_cols(newdat)
ggplot(res2, aes(x = mindiff.1, y = meandiff.2)) + 
...
indf <- indf %>% mutate(`202010perc` = percent_rank(`202010`))
ranperc <- function(df, groupsize, ptile) {
  indf %>% group_by(geno) %>% slice_sample(n=1) %>% ungroup() %>%
    mutate(group = case_when(`202010perc` < ptile ~ "bottom",
                             `202010perc` > (1 - ptile) ~ "top")) %>%
    drop_na(group) %>%
    group_by(group) %>%
    slice_sample(n = groupsize) %>%
    summarize(mean.1 = mean(`202010`), mean.2 = mean(value)) %>%
    summarize(across(2:3, diff))
}

res <- replicate(100, ranperc(indf, 3, 0.3), simplify = FALSE)
res2 <- map_dfr(res, bind_rows)
res2 <- res2 %>% 
