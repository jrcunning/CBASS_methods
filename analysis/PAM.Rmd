...
#   #slice(1:50)
#   #filter(std.error_2020 < 0.3634071 & std.error_2021 < 0.2960742) %>%
#   filter(sum.std.error < 0.5083034); top

#filtered2021-1 with sumse cutoff 0.677 (n=58) >> 0.303, 0.655
#filtered2021-1 with sumse cutoff 0.639 (top85%; n=56) >> 0.284, 0.642
#filtered2021-1 with sumse cutoff 0.5858 (top80%; n = 52) >> 0.285, 0.619
#filtered2021-2, new2020, sumse cutoff 0.57458(90%, n=60 >> 0.295, 0.768
#filtered2021-2, new2020, sumse cutoff 0.5323947(85%, n=57 >> 0.295, 0.768
#filtered2021-2, new2020, sumse cutoff 0.5083034(80%, n=53 >> 0.290, 0.662

# hist(longdf$std.error)
# toplongdf <- longdf %>% filter(geno != "CRF059")#%>%
#   #filter(geno %in% top$geno)




##### new filtering with all three datasets
out <- longdf %>%
  group_by(nursery, geno) %>%
  summarize(meanse = mean(std.error)) %>%
  filter(meanse > quantile(meanse, 0.9))

toplongdf <- longdf #%>% filter(!geno %in% out$geno)
```

# Plot and analyze unadjusted data
```{r}
# longdf <- longdf %>%
#   mutate(test = case_when(date == "202010" ~ "test1", 
#                           date %in% c("202006", "202109") ~ "test2"))

topwidedf <- toplongdf %>%
  pivot_wider(names_from = date, values_from = c(ed50, std.error))

ggplot(topwidedf, aes(x = ed50_202010)) +
  facet_wrap(~nursery) +
  # geom_point(data = topwidedf %>% group_by(nursery) %>% summarise_all(mean), 
  #            size = 3, color = "red", alpha = 0.5) + 
  geom_point(aes(y = ed50_202109, color = geno %in% out$geno), pch = 1) +
  geom_point(aes(y = ed50_202006), pch = 2) +
  geom_abline(slope = 1, intercept = 0, lty = 2) +
  #stat_cor() +
  coord_cartesian(xlim = range(toplongdf$ed50), ylim = range(toplongdf$ed50)) +
  theme_custom()
...
res.unadj <- toplongdf %>% 
  ...
  select(-test) %>%
  ungroup()
hist(abs(res.unadj$diff))



# # 2021 vs. 2020 using raw ED50s
# diffs <- topwidedf %>%
#   mutate(diff = ed50_2021 - ed50_2020)
# hist(abs(diffs$diff))

# Get ECDF for plotting
# res2 <- topwidedf %>%
#   mutate(diff = abs(ed50_2021 - ed50_2020))
dens <- density(abs(res.unadj$diff), adjust = 0.1)
...
res.adj <- out %>% 
  ...
  select(-test) %>%
  ungroup()
hist(abs(res.adj$diff))

# Get ECDF for plotting
# res2 <- out %>%
#   mutate(diff = abs(ed50_adj_2021 - ed50_adj_2020))
dens <- density(abs(res.adj$diff), adjust = 0.1)
dens <- tibble(x = dens$x, y = dens$y) %>%
  mutate(cd = cumsum(y)/sum(y))
# Get degree diffs for x% of cases from cumulative density
conf0.5 <- dens$x[which.min(abs(0.5 - dens$cd))]
...
ggplot(res.adj, aes(x=pair, y = abs(diff), color = nursery)) +
  geom_jitter()
```
