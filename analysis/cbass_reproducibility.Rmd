...
# ed50_2020_old <- read_csv("~/Projects/CBASS_FL_Acer/data/processed/ed50_values.csv") %>%
...
ed50_202010 <- read_csv("data/reproducibility/processed/ed50_202010.csv") %>%
  mutate(nursery = case_when(nursery == "RR" ~ "rrt", TRUE ~ tolower(nursery))) %>%
  select(nursery, geno = name, ed50 = ed50, std.error = std.error)

# # COmparing mysterious old data to 'new'
# ed50comp <- full_join(ed50_202010, ed50_2020_old, by = c("nursery", "geno"))
# ggplot(ed50comp, aes(x = ed50, y = ed50_2020, color = nursery)) + geom_point() + geom_label(aes(label = geno))
# # The 'new' data match to the original dose response curves in Rmarkdowns from the original project. So, dismiss the mysterious old data as erroneous....
...
### quick side-analysis of UM Acer to help them identify candidates for selective breeding
```{r}
um <- bind_rows(
  filter(ed50_202010, nursery == "um"),
  filter(ed50_202109, nursery == "um")
) %>%
  pivot_wider(names_from = date, values_from = c(ed50, std.error)) %>%
  rowwise() %>%
  mutate(mean_ed50 = mean(c(ed50_202010, ed50_202109), na.rm = TRUE))

genokey <- read_csv("~/Projects/CBASS_FL_Acer/data/genotype_name_key.csv")
um %>%
  left_join()
write_csv(um, path = "um_ed50s.csv")


kjs <- filter(ed50_202010, nursery == "nsu")
```
...
# longdf <- longdf %>% filter(!geno %in% outliers$geno)
```

# Plot and analyze unadjusted data
```{r}
# Total number of ED50s being analyzed
nrow(longdf)  # 174 CBASS ED50s
n_distinct(longdf$nursery, longdf$geno)  # 85 unique colonies/genos
longdf %>%
  count(date, nursery) %>%
  arrange(date)

# Identify and remove outliers
mod <- lm(ed50 ~ geno + nursery:date, data = longdf)
anova(mod)
ress <- augment(mod, longdf) %>% arrange(-.cooksd)
hist(ress$.cooksd, breaks = 80)
highcooks <- ress %>% filter(.cooksd > 0.035)
# Plot outliers
ggplot(longdf, aes(x = date, y = ed50, color = geno %in% highcooks$geno)) +
  geom_point() +
  geom_line(aes(group = geno))

# Remove outliers/genotypes with highest cooksD
toplongdf <- longdf %>% filter(!geno %in% highcooks$geno)
# How many remaining after filtering outliers
toplongdf %>% count(date)
n_distinct(toplongdf$geno)
# Refit model
mod.refit <- lm(ed50 ~ nursery:date + geno, data = toplongdf)
modres <- tidy(anova(mod.refit))
# Variance explained by genotype X environment
modres %>%
  mutate(varexpl = sumsq / sum(sumsq))



# Scatter plots of test-retest ED50s (unadjusted)
topwidedf <- toplongdf %>%
  pivot_wider(names_from = date, values_from = c(ed50, std.error))
means <- topwidedf %>% group_by(nursery) %>%
  summarize(across(starts_with("ed50"), mean, na.rm = TRUE))
unadj.corr <- ggplot(topwidedf, aes(x = ed50_202010)) +
  facet_wrap(~ nursery) +
  geom_point(data = means, aes(y = ed50_202109), pch = 16, size = 3, fill = "gray", alpha = 0.3) + 
  geom_point(data = means, aes(y = ed50_202006), pch = 17, size = 3, fill = "gray", alpha = 0.3) + 
  geom_abline(slope = 1, intercept = 0, lty = 1, lwd = 0.25) +
  geom_abline(data = means, aes(slope = 1, intercept = ed50_202109 - ed50_202010), 
              color = "gray", lty = 3) +
    geom_abline(data = means, aes(slope = 1, intercept = ed50_202006 - ed50_202010), 
              color = "gray", lty = 2) +
  ...
  theme_custom() +
  labs(x = "Unadjusted ED50 (°C) in Test 1", y = "Unadjusted ED50 (°C) in Test 2"); unadj.corr
...
myecdf <- ggplot(res.unadj, aes(x = abs(diff))) +
  geom_segment(aes(y = 0.5, yend = 0.5, x = 0, xend = conf0.5), 
               lty = 2, alpha = 0.4, lwd = 0.25, color = "gray") +
  geom_segment(aes(x = conf0.5, xend = conf0.5, y = 0, yend = 0.5), 
               lty= 2, alpha = 0.4, lwd = 0.25, color = "gray") +
  geom_segment(aes(y = 0.9, yend = 0.9, x = 0, xend = conf0.9), 
               lty = 2, alpha = 0.4, lwd = 0.25, color = "gray") +
  geom_segment(aes(x = conf0.9, xend = conf0.9, y = 0, yend = 0.9), 
               lty= 2, alpha = 0.4, lwd = 0.25, color = "gray") +
  #geom_rect(data = myvals, aes(xmin = 0, xmax = diff, ymin = 0, ymax = perc), alpha = 0.2) +
  stat_ecdf(color = "gray", alpha = 0.5) +
...
# Generate adjusted ED50s by removing variation due to nursery and date
```{r}
...
adj.corr <- out %>% pivot_wider(names_from = date, values_from = c(ed50, ed50_adj)) %>%
  ...
  labs(x = "Adjusted ED50 (°C) in Test 1", y = "Adjusted ED50 (°C) in Test 2") +
...
ecdf2 <- myecdf +
  geom_segment(aes(y = 0.5, yend = 0.5, x = 0, xend = adjconf0.5), lty = 2, lwd = 0.2) +
  geom_segment(aes(x = adjconf0.5, xend = adjconf0.5, y = 0, yend = 0.5), lty= 2, lwd = 0.2) +
  geom_text(x = adjconf0.5 - 0.01, y = 0.02, label = round(adjconf0.5, 2), adj = 1, size = 3) +
  geom_segment(aes(y = 0.9, yend = 0.9, x = 0, xend = adjconf0.9), lty = 2, lwd = 0.2) +
  geom_segment(aes(x = adjconf0.9, xend = adjconf0.9, y = 0, yend = 0.9), lty= 2, lwd = 0.2) +
  geom_text(x = adjconf0.9 + 0.01, y = 0.02, label = round(adjconf0.9, 2), adj = 0, size = 3) +
  #geom_rect(data = myvals, aes(xmin = 0, xmax = diff, ymin = 0, ymax = perc), alpha = 0.2) +
  stat_ecdf(data = res.adj, color = "black"); ecdf2


# Compare all diffs between raw and adjusted ED50s
summary(abs(res.unadj$diff))
summary(abs(res.adj$diff))
```
...
ggsave(repro.fig, filename = "output/Figure4.png", width = 183, height = 85, units = "mm")
```


# Reproducibility of rank order in retest, based on initial test ED50 difference

# How confident can we be in comparing 2 individuals or 2 groups of corals based on one CBASS test?
```{r}
####
...
# Get subset of genos spanning equal bins across range of oct2020ed50_adj distribution
...
probplot <- ggplot(allprob, aes(x = absdiff.1, y = fit)) +
  ...
  labs(x = "Difference in ED50 in initial test (°C)", y = "Probability of same rank order in re-test")
probplot

ggsave(probplot, filename = "output/probplot.png", width = 60, height = 60, units = "mm")


allprob %>% group_by(set, groupsize) %>%
  filter(fit > 0.9) %>%
  summarize(min(absdiff.1))

range(longdf$ed50)
```

# What if you pick top ten vs. bottom ten.
```{r}
df2 <- out %>%
  pivot_wider(names_from = "date", values_from = c(ed50, ed50_adj)) %>%
  select(geno, `202010` = ed50_202010, `202006` = ed50_202006, `202109` = ed50_202109) %>% 
...
# What if you pick N genos from top and X from bottom half from test 1, then retest
...
# Function to get tops and bottoms halfs and then compare them
reprun <- function(df, nsim, groupsize) {
  # Arrange input data in order of ED50 in Test 1
  df.ord <- df %>% arrange(-`202010`)
  # Get top half and bottom half of dataset
  tops <- slice_head(df.ord, n = floor(nrow(df.ord) / 2))
  bottoms <- slice_tail(df.ord, n = floor(nrow(df.ord) / 2))
  # Run comparisons
  replicate(nsim, ran(tops, bottoms, groupsize), simplify = FALSE)
}
# Function to compare 2 random groups of specified size from top vs. bot
ran <- function(tops, bottoms, groupsize) {
  t <- tops %>% slice_sample(n = groupsize)
  b <- bottoms %>% slice_sample(n = groupsize)
  g <- bind_rows(t, b) %>% mutate(group = c(rep(1, groupsize), rep(2, groupsize)))
  r <- suppressMessages(g %>% group_by(group) %>% 
    summarize(mean.1 = mean(`202010`), mean.2 = mean(value)) %>%
    summarize_all(diff) %>%
    mutate(absdiff.1 = abs(mean.1), meandiff.2 = mean.2 * sign(mean.1)))
...
library(furrr)
set.seed(8)
runres.unadj <- tibble(groupsize = c(1,3,5)) %>%
  mutate(runout = future_map(groupsize, ~reprun(indf.unadj, 10000, groupsize = .x)),
         runout = map(runout, ~map_dfr(.x, bind_rows)),
         fit = map(runout, ~lm(meandiff.2 ~ absdiff.1, data = .x)),
         pred = map(fit, ~data.frame(fit = predict(.x, newdata = data.frame(absdiff.1 = seq(0, 2, 0.01)),
...
# Run simulations on adjusted ED50 data
set.seed(8)
runres.adj <- tibble(groupsize = c(1,3,5)) %>%
  mutate(runout = future_map(groupsize, ~reprun(df = indf.adj, nsim = 10000, groupsize = .x)),
         runout = map(runout, ~map_dfr(.x, bind_rows)),
         fit = map(runout, ~lm(meandiff.2 ~ absdiff.1, data = .x)),
         pred = map(fit, ~data.frame(fit = predict(.x, newdata = data.frame(absdiff.1 = seq(0, 2, 0.01)),
                                                   interval = "prediction", level = 0.9))),
