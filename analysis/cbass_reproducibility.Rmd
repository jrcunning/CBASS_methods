...
# # Repeatability
# # library(rptR)
# Rout <- rpt(ed50 ~ nursery * date  + (1|geno), data = toplongdf, datatype = "Gaussian",
#            grname = c("geno", "Overdispersion", "Fixed", "Residual"),
#            nboot = 1000, CI = 0.90, parallel = TRUE, adjusted = TRUE)
# Rout
# plot(Rout)
# summary(Rout)
# 
# Rout <- rpt(ed50 ~ nursery * date  + (1 |geno), data = toplongdf, datatype = "Gaussian",
#            grname = c("geno", "Overdispersion", "Fixed", "Residual"),
#            nboot = 1000, CI = 0.90, parallel = TRUE, adjusted = FALSE, ratio = FALSE)
# Rout
# plot(Rout)
# summary(Rout)
```
...
runres.unadj <- tibble(groupsize = c(1,3,10)) %>%
  mutate(runout = future_map(groupsize, ~reprun(indf.unadj, 20000, groupsize = .x)),
...
runres.adj <- tibble(groupsize = c(1,3,10)) %>%
  mutate(runout = future_map(groupsize, ~reprun(indf.adj, 20000, groupsize = .x)),
         runout = map(runout, ~map_dfr(.x, bind_rows)),
         fit = map(runout, ~lm(meandiff.2 ~ absdiff.1, data = .x)),
         pred = map(fit, ~data.frame(fit = predict(.x, newdata = data.frame(absdiff.1 = seq(0, 2, 0.01)),
                                                   interval = "prediction", level = 0.9))),
