...
```{r}
# calculate variance of cbass vs. classic?
both %>% 
  distinct(date, genotype, estimate.ed50, estimate.dfvfm) %>%
  group_by(date) %>%
  summarize(cv.ed50 = sd(estimate.ed50)/mean(estimate.ed50),
            cv.dfvfm = sd(-estimate.dfvfm)/mean(-estimate.dfvfm))
