...
ggplot(fvals, aes(x = max_temp, y = .fitted, color = geno)) +
  geom_line() +
...
drc_ed50_fig <- drcs + 
  geom_histogram(data = fed50, inherit.aes = FALSE, alpha = 0.3,
...
comp <- ggplot(both, aes(x = estimate.ed50, y = estimate.dfvfm)) +
  ...
  stat_cor(method = "spearman", label.y = -0.08, label.x = 34.2, size = 2) + 
  stat_cor(method = "pearson", label.y = -0.10, label.x = 34.2, size = 2) +
  scale_x_continuous(breaks = seq(34, 37, 0.5), expand = c(0, 0)) +
  scale_y_continuous(expand = c(0, 0)) +
  coord_cartesian(xlim = c(34, 37), ylim = c(-0.55, -0.03)) +
  theme_custom() +
