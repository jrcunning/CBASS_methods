...
ggplot(fvals, aes(x = max_temp, y = fvfmraw)) +
  facet_wrap(~geno) +
  geom_point(aes(shape = problem)) +
  geom_line(aes(y = .fitted), data = drop_na(fvals)) +
  geom_line(aes(y = .fitted), data = vals, lty = 2) +
  geom_vline(aes(xintercept = estimate), data = fed50, lty = 1, lwd = 0.25) +
  geom_vline(aes(xintercept = estimate), data = ed50, lty = 2, lwd = 0.25) +
...
drcs <- ggplot(test2, aes(x = max_temp, y = .fitted, group = geno)) +
...
ed50hist <- ggplot(fed50, aes(x = estimate)) +
  geom_histogram(alpha = 0.5, position = "stack", breaks = seq(32,38,0.25)) +
  scale_x_continuous(limits = c(32, 38), expand = c(0, 0)) +
  theme_custom(); ed50hist

drc_ed50_fig <- drcs + 
  geom_histogram(data = fed50, inherit.aes = FALSE, alpha = 0.5,
                 breaks = seq(32,38,0.25),
                 aes(x = estimate, y = ..count../12.5)) +
  scale_y_continuous(sec.axis = sec_axis(trans=~.*12.5, name = "Count of genet ED50s")) +
...
comp <- ggplot(both, aes(x = estimate.ed50, y = estimate.dfvfm)) +
  ...
  stat_cor(method = "spearman", label.y = -0.04, label.x = 34.3, size = 2) + 
  stat_cor(method = "pearson", label.y = -0.06, label.x = 34.3, size = 2) +
