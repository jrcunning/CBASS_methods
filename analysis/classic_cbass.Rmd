...
alldrc <- ggplot(fvals, aes(x = max_temp, y = fvfmraw)) +
  ...
  geom_line(aes(y = .fitted), data = drop_na(fvals)) +
...
test <- fmods %>%
  mutate(pred = map(ll3, ~augment(.x, newdata = data.frame(max_temp = seq(28.5, 40, 0.05))))) %>%
  select(geno, pred) %>%
  unnest(pred) %>%
  mutate(geno = factor(geno))

drcs <- ggplot(test, aes(x = max_temp, y = .fitted, group = geno)) +
...
ed50hist <- ggplot(fed50, aes(x = estimate)) +
  geom_histogram(alpha = 0.5, position = "stack", breaks = seq(32,38,0.5)) +
  scale_x_continuous(limits = c(32, 38), expand = c(0, 0)) +
...
#try_read_delim <- possibly(read_delim, otherwise = NULL)

pam1 <- pamfiles %>%
  map_dfr(read_delim, delim = ";", .id = "file_id")
#map means to take code and apply it to every file that's listed in pamfiles and store it into pam1. delim = delimiter. ; inbetween every value
...
pam3 %>% filter(date == "2022-12-06", file_id == "23_aoi.csv")
...
pam4 <- pam3 %>%
  ...
  group_by(file_id, date) %>%
  select(file_id, date, time, aoi, var, value)
    # Multiple data sets within each file, organized by file, AOI, variable and value

pam5 <- pam4 %>%
  separate(file_id, into = c("rack_no", "junk")) %>%
  mutate(rack_no = as.numeric(rack_no))
    # separate() used to turn a single character column into multiple columns -> "file_id" into "rack_no" and "junk". rack_no changed into numeric rather than character
...
pam6 <- pam5 %>%
  ...
  mutate(position = ceiling(as.numeric(aoi)/2)) %>%
  ungroup()
    # New column known as "position"[numeric] in order to place AOI datasets with same coral fragment

pam7 <- full_join(pam6, md, by = c("date", "rack_no", "position"))
    # left_join() -> includes all rows from first (left) table, but only rows from second (right) table that match the first table. Need to join to full_join() to include pam6 & md (metadata)
```
...
# Get control-heated diffs for each genotype with std error
finalpam <- pam8 %>%
  filter(var == "y_ii_") %>%
...
# Plot
myplot <- ggplot(finalpam, aes(x = date, y = value, color = genotype, asp = 1)) +
...
myplot +
  geom_line(aes(y = fvfm_avg, linetype = treatment), data = fvfm_summary)


library(lme4)
mod <- lmer(value ~ treatment * genotype + (1|tank) + (1|frag), data = finalpam)
car::Anova(mod)
library(emmeans)
...
```{r}
cbass_classic_results <- plot_grid(drc_ed50_fig, classicresfig, nrow = 1, labels = "auto")
ggsave(cbass_classic_results, filename = "output/cbass_classic_results.png", 
...
both <- full_join(cbass, classic, by = "genotype", suffix = c(".ed50", ".dfvfm"))

# Linear model / OLS
mod <- lm(estimate.dfvfm ~ estimate.ed50, data = both)
anova(mod)

# Passing Bablok regression
library(mcr)
PBreg <- mcreg(both$estimate.ed50, -both$estimate.dfvfm,
...
# Plot
comp <- ggplot(both, aes(x = estimate.ed50, y = estimate.dfvfm)) +
